...
void MAGIC_ClassificationApplication( TString myMethodList = "" )
{
   ...
   if (Use["MLP"])
   {
     ...
     hist_g      = new TH1F( "hist_g" ,           "hist_g" ,           nbin, -1.25, 1.5 );
     hist_h      = new TH1F( "hist_h" ,           "hist_h" ,           nbin, -1.25, 1.5 );
   }
   ...
   //
   std::cout << "--- Select validation sample" << std::endl;
   ...
   std::cout << "--- Processing: " << validation_tree->GetEntries() << " events" << std::endl;
   TStopwatch sw;
   sw.Start();
   for (Long64_t ievt=0; ievt<validation_tree->GetEntries();ievt++) {
      ...
      if (Use["MLP"          ])
      {
        ...
        if(id==1)
        hist_g     ->Fill( reader->EvaluateMVA( "MLP method"           ) );
        else
        hist_h     ->Fill( reader->EvaluateMVA( "MLP method"           ) );
      }
   ...
   if (Use["CutsGA"]) std::cout << "--- Efficiency for CutsGA method: " << double(nSelCutsGA)/validation_tree->GetEntries()
   ...
   std::cout << "--- Created root file: \"Application_output.root\" containing the MVA output histograms" << std::endl;
...
int main( int argc, char** argv )
{
   ...
   MAGIC_ClassificationApplication(methodList);
   return 0;
}
